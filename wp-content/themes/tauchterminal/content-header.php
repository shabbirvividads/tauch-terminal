...
<?php if (have_posts()) : ?>
    ...
    // Otherwise if we have a featured image, try to get text from the image
        ...
        $title = __('Portfolio', 'tauchterminal');
        ...
        if ($testimonial_options) {
            ...
            $title = __('Testimonials', 'tauchterminal');
        }
        ...
        the_post();
        $title = sprintf(__('Author: %s', 'tauchterminal'), '<span class="vcard">' . get_the_author() . '</span>');
        ...
        $title = sprintf(__('Search Results for: %s', 'tauchterminal'), '<span>' . get_search_query() . '</span>');

    } elseif (is_day()) {
        $title = sprintf(__('Day: %s', 'tauchterminal'), '<span>' . get_the_date() . '</span>');

    } elseif (is_month()) {
        $title = sprintf(__('Month: %s', 'tauchterminal'), '<span>' . get_the_date('F Y') . '</span>');

    } elseif (is_year()) {
        $title = sprintf(__('Year: %s', 'tauchterminal'), '<span>' . get_the_date('Y') . '</span>');

    } elseif (is_tax('post_format', 'post-format-aside')) {
        $title = __('Asides', 'tauchterminal');

    } elseif (is_tax('post_format', 'post-format-image')) {
        $title = __('Images', 'tauchterminal');

    } elseif (is_tax('post_format', 'post-format-video')) {
        $title = __('Videos', 'tauchterminal');

    } elseif (is_tax('post_format', 'post-format-quote')) {
        $title = __('Quotes', 'tauchterminal');

    } elseif (is_tax('post_format', 'post-format-link')) {
        $title = __('Links', 'tauchterminal');

    } else {
        $title = __('Archives', 'tauchterminal');
